...
`passed` は真偽値型の重要なプロパティで、部品を出荷するか、再加工するかを決めます。`defectCode` プロパティは、不合格の原因を分類して根本原因分析に役立てます。
...
> **フィードバックループ：** 品質検査で不合格になると、`Quality-Check (passed=false) → Part → Work-Order → Machine` の順に生産チェーンを遡れます。このフィードバックループにより、スマート工場は問題のある機械を特定し、継続的に生産品質を改善できます。
