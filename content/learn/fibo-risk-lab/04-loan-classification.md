...
```quiz
Q: 建設ローンのBaselリスクウェイトが150%である一方、SBAローンの米国政府による無条件保証部分が0%なのはなぜですか？
- 建設ローンは手続きに時間がかかるため
- SBAローンの米国政府による無条件保証部分では銀行が信用リスクを負わない一方、建設ローンはデフォルトリスクと完成リスクが高いため [correct]
- 建設会社の収益性が低いため
- SBAがSafe Banking Assetの略であるため
> Basel IIIのリスクウェイトは、銀行が負担する信用リスクを反映します。このモデルでは、SBA（Small Business Administration）ローンのうち米国政府による無条件保証の対象部分を信用リスク0%として扱います。建設ローンには完成リスク、市場リスク、高いデフォルト率があるため、150%のリスクウェイトを適用したリスク加重資産に応じて自己資本を備える必要があります。
```
